...
bool Graph<T>::removeVertex(const T &in) {
	...
	for (auto vit = vertexSet.begin(); vit != vertexSet.end(); vit++){
	    if (*vit == v)
	        vit = vertexSet.erase(vit);
	    else {
	        for (auto eit = (*vit)->adj.begin(); eit != (*vit)->adj.end() and !(*vit)->adj.empty(); eit++){
	            if (eit->dest == v)
                    eit = (*vit)->adj.erase(eit);
	        }
	    }
	}
	return true;
...
vector<T> Graph<T>::dfs() const {
	...
	vector<T> res;
	if (!vertexSet.empty()){
	    vertexSet[0]->visited = true;
	    dfsVisit(vertexSet[0], res);
	}
	for (Vertex<T> *v: vertexSet){
        if (!v->visited) res.push_back(v->info);
	}
	return res;
}
...
vector<T> Graph<T>::bfs(const T & source) const {
	...
	vector<T> res;
	queue<Vertex<T>*> tmp; tmp.push(vertexSet[0]);
	while(!tmp.empty()){
	    for (Edge<T> e: tmp.front()->adj){
	        if (!e.dest->visited) {
                tmp.push(e.dest);
                e.dest->visited = true;
            }
	    }
	    res.push_back(tmp.front()->info);
	    tmp.pop();
	}
	return res;
}
